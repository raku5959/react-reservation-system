...
export default function ReservationEdit() {
  ...
  async function loadData(reservationId: string) {
    setLoading(true);
    setError("");
    ...
    try {
      ...
      if (reservationResult.error) {
        throw new Error(
          `予約取得エラー: ${reservationResult.error.message}`
        );
      }

      if (
        !reservationResult.data ||
        reservationResult.data.length === 0
      ) {
        throw new Error(
      ...
      if (therapistResult.error) {
        throw new Error(
          `セラピスト取得エラー: ${therapistResult.error.message}`
        );
      }

      if (courseResult.error) {
        throw new Error(
          `コース取得エラー: ${courseResult.error.message}`
        );
      }
      ...
      const data = reservationResult.data[0];
  ...
  const selectedCourse = courses.find(
    (course) =>
      String(course.id) === form.course_id
  );
  ...
  async function handleSubmit(
    ...
    try {
      ...
      if (updateError) {
        ...
        throw new Error(
          `予約の更新に失敗しました: ${updateError.message}`
        );
      }
